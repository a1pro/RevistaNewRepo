...
const ProductDetails: React.FC<Props> = ({ route, navigation }) => {
    ...
    let imageSource;
    if (product.images && product.images.length && product.images[0].startsWith('http')) {
  ...
  const handleAddToWishlist = async () => {
        try {
            ...
            if (res.data && res.data.message === "Successfully added!") {
                ...
                Alert.alert('Success', res.data.message || 'Product added to wishlist!');
            } else {
                Alert.alert('Error', res.data.message || 'Failed to add to wishlist.');
            }
        } catch (error) {
            Alert.alert('Error', error?.response?.data?.message || 'Error adding to wishlist.');
        } finally {
            setWishlistLoading(false);
        }
    };
  ...
  const handleRemoveFromWishlist = async () => {
        try {
            ...
            if (res.data && (res.data === "successfully removed!" || res.data.message === "successfully removed!")) {
                ...
                Alert.alert('Error', res.data?.message || 'Failed to remove product from wishlist.');
            }
        } catch (error) {
            Alert.alert('Error', error?.response?.data?.message || 'Failed to remove product from wishlist.');
        } finally {
            setWishlistLoading(false);
        }
    };
  const handleAddToCart = async () => {
        try {
            ...
            Alert.alert('Error', error?.response?.data?.message || 'Error adding to cart.');
        } finally {
            setCartLoading(false);
        }
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            <ScrollView>
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.row}>
                        ...
                        <TouchableOpacity onPress={openReviewsModal}>
                            <Text style={[styles.reviewText, { textDecorationLine: 'underline', color: '#1663F7' }]}>
                                ({reviews.length || reviewsCount} {reviews.length === 1 || reviewsCount === 1 ? 'review' : 'reviews'})
                            </Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={localStyles.reviewBtn} onPress={openReviewModal}>
                    ...
                    <Text style={styles.price}>
                        {product.unit_price?.toFixed(2)} <Text style={styles.currency}>﷼</Text>
                    </Text>
                    <Text style={styles.sectionTitle}>Details</Text>
                    <Text style={styles.detailsText}>
                        {product.details ? product.details.replace(/<[^>]+>/g, '') : 'No details available.'}
                    </Text>
                ...
                {similarLoading ? (
                    <ActivityIndicator size="small" color="#1663F7" style={{ marginVertical: 16 }} />
                ) : similarProducts.length === 0 ? (
                    <Text style={{ color: '#888', marginVertical: 8, marginLeft: 30 }}>No similar product.</Text>
                ) : (
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginVertical: 8 }}>
                        {similarProducts.map((item, idx) => (
                            ...
                            >
                                <Image
                                    source={
                                        item.images && item.images.length && item.images[0].startsWith('http')
                                            ? { uri: item.images[0] }
            ...
            <View style={styles.bottomBar}>
                ...
                <TouchableOpacity style={styles.buyBtn} onPress={() => navigation.navigate('Address')}>
            ...
            >
                ...
                <View style={localStyles.halfModalContainer}>
                    ...
                    <Text style={localStyles.orderText}>Order #{product.order_number || 'xxxxxxx'}</Text>
            ...
            >
                ...
                <View style={localStyles.halfModalContainer}>
                    <View style={{ alignItems: 'center', marginBottom: 10 }}>
                        <View style={{ width: 40, height: 5, backgroundColor: '#ccc', borderRadius: 2.5, marginBottom: 10 }} />
                        <Text style={localStyles.modalTitle}>Product Reviews</Text>
                    </View>
                    <ScrollView style={{ flex: 1 }}>
                        {reviewsLoading ? (
                            <ActivityIndicator size="large" color="#1663F7" style={{ marginTop: 40 }} />
                        ) : reviews.length === 0 ? (
                            <Text style={{ textAlign: 'center', marginTop: 30, color: '#888' }}>No reviews yet.</Text>
                        ) : (
                            reviews.map((review, idx) => (
                                <View key={review?.id || idx} style={localStyles.reviewCard}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
                                        <Image
                                            source={
                                                review?.customer?.image && review?.customer.image !== 'def.png'
                                        ...
                                        <View style={{ marginLeft: 12 }}>
                                            ...
                                            <Text style={localStyles.reviewEmail}>{review?.customer?.email || ''}</Text>
                                        </View>
                                    </View>
                                    ...
                                    <Text style={localStyles.reviewComment}>{review?.comment}</Text>
                                </View>
                            ))
                        )}
                    </ScrollView>
